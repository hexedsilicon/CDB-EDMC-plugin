# -*- coding: utf-8 -*-
#
# Station display and CDB lookup
# Adapted from the EDDB plugin
#
...
def station_url(text):
    if text:
        ...
        if station:
            return 'https://cdb.sotl.org.uk/stations/%d' % station

        system = system_id(this.system)
        if system:
            return 'https://cdb.sotl.org.uk/systems/%d' % system
...
def plugin_app(parent):
    this.station_label = tk.Label(parent, text = _('CDB') + ':')	# Main window
